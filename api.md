...
在 `el` 上绑定一个单次执行的处理器（触发并执行后自动解绑）。各个参数的含义与 `$.on()` 接口相同。
...
- `end: Element | String` 上层终止根元素（上溯的终点，不含）或其选择器。
- `slp: String` 纵向（父子级）路径匹配选择器，可选。
- `slr: String|[String]` 同级参考选择器（集），可选。即位置计数参考的匹配集。

> **注记：**<br>
> 可用于辅助元素定位，如从大纲目录中提取目标元素的层次逻辑，构造其选择器。<br>
